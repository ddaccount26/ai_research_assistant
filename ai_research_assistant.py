...
def make_zip_bytes(paths, base_dir: str = "."):
    buf = io.BytesIO()
    ...
    with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for p in paths:
            ...
            if pth.exists() and pth.is_file():
                try:
                    arcname = str(pth.resolve().relative_to(base))
                except Exception:
                    arcname = pth.name
...
if "downloads_ready_at" not in st.session_state:
    st.session_state.downloads_ready_at = None
if "top_3_papers" not in st.session_state:
    st.session_state.top_3_papers = None
...
# ---- File viewer / download (Community Cloud) ----
with st.sidebar:
    st.markdown("### Downloads & Files")
    data_dir = output_dir
    pdf_dir = os.path.join(output_dir, "pdfs")

    # Gate: do NOT show files until build is complete
    if not st.session_state.get("show_downloads", False):
        st.info("Click **Build Dataset** to generate new PDFs + metadata for download.")
    else:
        # Show quick status (only after build)
        if st.session_state.get("downloads_ready_at"):
            st.caption(f"Last build: {st.session_state.downloads_ready_at}")

        st.write("Data dir:", os.path.abspath(data_dir))
        st.write("PDF dir:", os.path.abspath(pdf_dir))

        # Optional manual refresh (mostly useful if you add other file-writing actions)
        if st.button("Refresh file list"):
            st.session_state["_refresh_files"] = str(datetime.now())

        # List only PDFs + metadata.csv (hide chunks.pkl, faiss.index)
        pdf_files = list_files_recursive(pdf_dir)
        metadata_path = os.path.join(output_dir, "metadata.csv")

        visible_files = pdf_files.copy()
        if os.path.exists(metadata_path):
            visible_files.append(metadata_path)

        if visible_files:
            st.write(f"Files available: {len(visible_files)}")
            st.code("\n".join(visible_files[:200]))
            if len(visible_files) > 200:
                st.caption("Showing first 200 files.")
        else:
            st.warning("Build finished, but no PDFs/metadata were found.")

        # Download a ZIP of PDFs + metadata.csv only
        artifacts = []
        artifacts += pdf_files
        if os.path.exists(metadata_path):
        ...
        if artifacts:
            zip_buf = make_zip_bytes(artifacts, base_dir=".")
            st.download_button(
                label="Download PDFs + metadata (.zip)",
                data=zip_buf,
                file_name="streamlit_data_export.zip",
                mime="application/zip",
            )

# Display top 3 papers if they exist in session state
if st.session_state.get("top_3_papers") is not None:
    st.markdown("### Top 3 Matching Papers")
    for rank, (paper, score) in enumerate(st.session_state.top_3_papers[:3], 1):
        with st.container():
...
if build_button:
    if not selected_keywords_list and not custom_keywords:
        ...
        # Reset download state at the start of build
        st.session_state.show_downloads = False
        st.session_state.downloads_ready_at = None
        st.session_state.top_3_papers = None
        ...
        try:
            ...
            if papers:
                ...
                # Store top 3 in session state
                st.session_state.top_3_papers = papers[:3]
                ...
                if success:
                    ...
                    # Now mark downloads as ready
                    st.session_state.show_downloads = True
                    st.session_state.downloads_ready_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    ...
                    # Rerun to refresh the UI with updated session state
                    st.rerun()
            else:
